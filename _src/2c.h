...
std::vector<std::string> formatToLinesAsEscapedStrings(const std::string &text, const AppConfig &appCfg)
{
    ...
    // Надо автоматически перенести данные на новую строку, если длина текущей дошла до лимита
    // Но если у нас CR на конце, то наверно за ним может идти LF, и тогда допустим единичный выход за размер
    // По символу LF переводим всегда
